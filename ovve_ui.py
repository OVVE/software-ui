...
from ui_settings import UISettings, TextSetting, FancyButtonSettings, SimpleButtonSettings, DisplayRectSettings


# TODO: Abstract text placement
class FancyDisplayButton(QAbstractButton):
    def __init__(self, label, value, unit, button_settings, parent=None, size = (200,100)):
...
class DisplayRect(QWidget):
    def __init__(self, label, value, unit, rect_settings, parent = None, size = (200,100)):
        super().__init__(parent)
        self.label = label
        self.value = value
        self.unit = unit
...
class MainWindow(QWidget):
    ...
    def makeFancyDisplayButton(self, label, value, unit, size = None):
        ...
        return FancyDisplayButton(label, value, unit, parent = None, size = s, button_settings=self.ui_settings.fancy_button_settings)
    ...
    def makeSimpleDisplayButton(self, value, size = None):
        ...
        return SimpleDisplayButton(value, parent = None, size = s, button_settings=self.ui_settings.simple_button_settings)
    ...
    def makeDisplayRect(self, label, value, unit, size = None):
        ...
        return DisplayRect(label, value, unit, parent = None, size = size, rect_settings=self.ui_settings.display_rect_settings)
    ...
    def initializeWidget1(self): #home screen
        ...
        self.mode_button_main = self.makeSimpleDisplayButton(self.settings.get_mode_display(), size = (150, 25))
        self.mode_button_main.clicked.connect(lambda: self.display(1))

        self.resp_rate_button_main = self.makeFancyDisplayButton("Resp. Rate", self.settings.resp_rate, "b/min", size = (150, 80))
        self.resp_rate_button_main.clicked.connect(lambda: self.display(2))

        self.minute_vol_button_main = self.makeFancyDisplayButton("Minute Volume", self.settings.minute_volume, "l/min", size = (150, 80))
        self.minute_vol_button_main.clicked.connect(lambda: self.display(3))

        self.ie_button_main = self.makeFancyDisplayButton("I/E Ratio", self.settings.get_ie_display(), "l/min", size = (150, 80))
        self.ie_button_main.clicked.connect(lambda: self.display(4))

        self.peep_display_main = self.makeDisplayRect("PEEP", 5, "cmH2O", size=(150, 80))
        self.tv_insp_display_main = self.makeDisplayRect("TV Insp", self.params.tv_insp, "mL", size=(150, 80))
        self.tv_exp_display_main = self.makeDisplayRect("TV Exp", self.params.tv_exp, "mL", size=(150, 80))
        self.ppeak_display_main = self.makeDisplayRect("Ppeak", self.params.ppeak, "cmH2O", size=(150, 80))
        self.pplat_display_main = self.makeDisplayRect("Pplat", self.params.pplat, "cmH2O", size=(150, 80))
    ...
    def initializeWidget2(self): #Mode
        ...
        self.mode_page_rect = self.makeDisplayRect("Mode", self.local_settings.get_mode_display(), "", size = (500,200))
    ...
    def initializeWidget3(self): #Resp_rate
        ...
        self.resp_rate_page_rect = self.makeDisplayRect("Resp. Rate", self.local_settings.resp_rate, "b/min", size = (500,200))

        resp_rate_increment_button = self.makeSimpleDisplayButton("+ " + str(self.settings.resp_rate_increment))
        resp_rate_decrement_button = self.makeSimpleDisplayButton("- " + str(self.settings.resp_rate_increment))
    ...
    def initializeWidget4(self): #Minute volume
        ...
        self.minute_vol_page_rect = self.makeDisplayRect("Minute Volume", self.local_settings.minute_volume, "l/min", size = (500,200))

        minute_vol_increment_button = self.makeSimpleDisplayButton("+ " + str(self.settings.minute_volume_increment))
        minute_vol_decrement_button = self.makeSimpleDisplayButton("- " + str(self.settings.minute_volume_increment))
    ...
    def initializeWidget5(self): #ie ratio
        ...
        self.ie_page_rect = self.makeDisplayRect("I/E Ratio", self.settings.get_ie_display(),"", size = (500,200))

        ie_change_size = (150,50)

        ie_change_0 = self.makeSimpleDisplayButton(self.settings.ie_ratio_display[0], size = ie_change_size)
        ie_change_1 = self.makeSimpleDisplayButton(self.settings.ie_ratio_display[1], size = ie_change_size)
        ie_change_2 = self.makeSimpleDisplayButton(self.settings.ie_ratio_display[2], size = ie_change_size)
        ie_change_3 = self.makeSimpleDisplayButton(self.settings.ie_ratio_display[3], size = ie_change_size)
